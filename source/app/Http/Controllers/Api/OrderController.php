  ...
  public function delete_order(Request $request)
  {
      $cart_id = $request->cart_id;
       $user = DB::table('orders')
        ...
        if($user->order_status == 'Completed' || $user->order_status == 'completed' || $user->order_status == 'COMPLETED'){
            
              $message = array('status'=>'0', 'message'=>'You can not cancel this order as this order has been completed');
              return $message;
       ...
       if($order){
        ...
        if($user->payment_method == 'COD' || $user->payment_method == 'Cod' || $user->payment_method == 'cod'){
            $newbal1 = $userwa1->wallet + $user->paid_by_wallet;  
			 $userwalletupdate = DB::table('users')
                ...
                $his = DB::table('wallet_recharge_history')
                    ...
                    'payment_gateway'=>"Order ID (".$cart_id." )"]);
              }               
              }
             ...
             if($user->paid_by_wallet >0){  
              ...
              $result = curl_exec($ch);
                ...
                $his = DB::table('wallet_recharge_history')
                    ...
                    $newbal1 = $userwa1->wallet + $user->paid_by_wallet + $user->rem_price; 
                     $userwalletupdate = DB::table('users')
                    ->where('id',$user_id1)
                     ->update(['wallet'=>$newbal1]);  
 		 ...
 		 foreach($v as $vs){
            ...
            ->join('product','product_varient.product_id','=','product.product_id')
           ->where('store_products.varient_id',$vs->varient_id)
           ->where('store_products.store_id',$user->store_id)
           ->first();
